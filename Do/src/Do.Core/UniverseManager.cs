...
namespace Do.Core {
	...
	public class UniverseManager {
		...
		private void UpdateItemSources ()
		{
			...
			t_update = 0;
			sources = PluginManager.GetItemSources ();
			...
			while (t_update < MaxUpdateTime / 2) {
				...
				Log.Info ("Updated \"{0}\" Item Source ({1} items).",
						source.Name, source.Items.Count);
		...
		{
			firstResults.Clear ();
			// For each starting character, add every matching object from the
			// universe to the firstResults list corresponding to that
			// character.
		...
		public void Search (ref SearchContext context)
		{
			...
			if (oldContext != null) {
				...
				// TODO: Children are not filtered at all. This needs to be
				// fixed.
		...
		private SearchContext ChildContext (SearchContext context)
		{
			...
			// We need to do something like this (filter the children), but DR's
			// work is intractable.
			// if (!context.Independent) {
		...
		private List<IObject> DependentResults (SearchContext context)
		{
			...
			if (context.ActionSearch && context.Query.Length == 0) {
				...
				return FilterPreviousWithContinuedContext (context);
			}
			...
			if (context.ModifierItemsSearch) {
				// Use a dictionary to get the intersection of the dynamic
				// modifier items for all items in the context.
				Dictionary <IItem, IItem> dynamicModItems =
					new Dictionary<IItem, IItem> ();
		...
		private List<IObject> SpecialItemsForContext (SearchContext context)
		{
			...
			if (context.ModifierItemsSearch) {
				if (context.Action.SupportsModifierItemForItems (
					context.Items.ToArray (), textItem))
		...
		// This generates a list of modifier items supported by the context in a
		// given initial list.
		public List<IObject> GetModItemsFromList (SearchContext context,
												  List<IObject> initialList)
		{
			List<IObject> results = new List<IObject> ();
			...
			foreach (IObject iobject in initialList) {
				if (iobject is IItem) {
					...
					if (context.Action.SupportsModifierItemForItems (items,
						iobject as IItem)) {
						results.Add (iobject);
					}
				}
		...
		public List<IObject> GetItemsFromList (SearchContext context,
											   List<IObject> initialList)
		{
			List<IObject> results = new List<IObject> ();
			foreach (IObject iobject in initialList) {
		...
		// This will filter out the results in the previous context that match
		// the current query
		...
		// This method gives us all the actions that are supported by all the
		// items in the list
		private List<IObject> InitialActionResults (SearchContext context)
		{
			...
			foreach (IItem item in context.Items) {
				...
				// If this is the first item in the list, add all of its
				// supported actions.
				if (initial) {
					...
					// pre-existing list if its not supported by this item,
		...
		public List<IObject> ActionsForItem (IItem item)
		{
			...
			foreach (IAction action in PluginManager.GetActions ()) {
