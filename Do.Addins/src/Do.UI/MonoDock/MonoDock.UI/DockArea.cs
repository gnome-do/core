...
namespace MonoDock.UI
{
	...
	public class DockArea : Gtk.DrawingArea
	{
		...
		public int Width {
			get {
				return (DockItems.Count * IconSize) + Preferences.ZoomSize + 2*XBuffer;
			}
		}
		...
		double DockIconOpacity {
			get {
				double total_time = (DateTime.UtcNow - State.GetPaneResultsTime (CurrentPane)).TotalMilliseconds;
				return Math.Min (1, Math.Pow((2*total_time)/(BaseAnimationTime*2)-1,2));
			}
		}
		
		IList<DockItem> DockItems {
			get {
				if (!input_interface || (State.CurrentPane == Pane.First && string.IsNullOrEmpty (State.FirstQuery))) {
					return dock_items;
				} else if ((DateTime.UtcNow-State.GetPaneResultsTime (CurrentPane)).TotalMilliseconds < BaseAnimationTime) {
					if (State.GetPanePreviousResults (CurrentPane) == null)
						return dock_items;
					return State.GetPanePreviousResultsAsDockItems (CurrentPane, dock_items.Count);
				}
				return State.GetPaneResultsAsDockItems (CurrentPane,  dock_items.Count);
			}
		}
		...
		Gdk.Point Cursor {
			get {
				if (!input_interface)
					return cursor;
				int current_cursor = IconNormalCenterX (State.GetPaneCursor (CurrentPane));
				int previous_cursor = IconNormalCenterX (State.GetPanePreviousCursor (CurrentPane));
				double current_x = previous_cursor + (current_cursor-previous_cursor)*CursorSlidePercentage;
				return new Gdk.Point ((int) current_x, Height - 10);
			}
			set {
		...
		double CursorSlidePercentage {
			get { return Math.Min (1,(DateTime.UtcNow - State.GetPaneCursorTime (CurrentPane)).TotalMilliseconds / BaseAnimationTime); }
		}
		...
		bool ResultsChangeAnimationNeeded {
			get { return (DateTime.UtcNow - State.GetPaneResultsTime (CurrentPane)).TotalMilliseconds < 2*BaseAnimationTime; }
		}
		
		bool CursorChangeAnimationNeeded {
			get {
				return (DateTime.UtcNow - State.GetPaneCursorTime (CurrentPane)).TotalMilliseconds < BaseAnimationTime;
			}
		}
		
		bool AnimationNeeded {
			get { return PaneChangeAnimationNeeded || ZoomAnimationNeeded || 
				BounceAnimationNeeded || ResultsChangeAnimationNeeded || CursorChangeAnimationNeeded; }
		}
		#endregion
		...
		void DrawDrock (Context cr)
		{
			...
			if (DockIconOpacity > 0) {
				...
				cr.SetSource (dock_icon_buffer, 0, 0);
				cr.Paint ();
			}
		}
		...
		void DrawIcons (Context cr)
		{
			for (int i=0; i<DockItems.Count; i++) {
				...
				Gdk.CairoHelper.SetSourcePixbuf (cr, DockItems[i].Pixbuf, x*DockItem.IconQuality, y*DockItem.IconQuality);
		...
		protected override bool OnButtonReleaseEvent (Gdk.EventButton evnt)
		{
			...
			if ((DateTime.UtcNow - DockItems[item].LastClick).TotalMilliseconds > BounceTime) {
				...
				IItem doItem = DockItems[item].IObject as IItem;
				if (doItem != null)
					window.Controller.PerformDefaultAction (DockItems[item].IObject as IItem);
				AnimatedDraw ();
			}
			return base.OnButtonReleaseEvent (evnt);
		}
